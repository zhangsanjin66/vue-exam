...
const routes = [
  ...
  },
  {
    ...
    // this generates a separate chunk (about.[hash].js) for this route
    // which is lazy-loaded when the route is visited.
    component: () => import(/* webpackChunkName: "register" */ '../views/NotFount.vue')
  },
]
...
// 在跳转路由之前，循环每一个路由
// 该方法接收一个回调函数,该回调函数接收两个参数
 // from从哪一个路由来
  // to 准备到哪个路由去
    // return  接收的是一个路由地址
    // router.addRoute('home',this.$store.state.rout)
    
    router.beforeEach(async (to,from,next) => {
      if(!store.state.user.userInfo){
        await store.dispatch('getUserInfoApi')
        console.log(store.state.user.userInfo);
      }
      // router.addRoute('home',store.state.user.rout)
      // ...
      // 返回 false 以取消导航
